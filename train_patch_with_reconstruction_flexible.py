"""
train_patch_with_reconstruction_flexible.py

MODIFIED: Supports both SimCLR pretraining AND random initialization
Use --pretrained-checkpoint for SimCLR, omit it for random init baseline
...
def main():
    ...
    parser.add_argument('--pretrained-checkpoint', type=str, default=None,
                       help='Path to SimCLR pretrained checkpoint. If None, use random init.')
    ...
    exp_dir = Path(args.output_dir) / init_method / f'fold_{args.fold}' / f'patch_recon_{timestamp}'
    ...
    # Load encoder (pretrained OR random)
    ...
    if args.pretrained_checkpoint:
        ...
        print(f"✓ Loaded SimCLR pretrained from epoch {checkpoint['epoch'] + 1}")
        encoder = simclr_model.encoder
    else:
        print("="*70)
        print("USING RANDOM INITIALIZATION (BASELINE)")
        print("="*70)
        print("✓ Encoder initialized with random weights (Kaiming normal)")
        # Encoder already has default PyTorch initialization
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.validate_recon_every == 0 or epoch == 0:
            print("\n" + "="*70)
            print(f"FULL RECONSTRUCTION VALIDATION ({init_method}, Fold {args.fold}, Epoch {epoch+1})")
            print("="*70)
        ...
        print(f"\n{'='*70}")
        print(f"{init_method} - Fold {args.fold} - Epoch {epoch+1}/{args.epochs}")
        print(f"{'='*70}")
    ...
    print("\n" + "="*70)
    print(f"{init_method} - FOLD {args.fold} TRAINING COMPLETE!")
    print("="*70)
