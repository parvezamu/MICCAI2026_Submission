...
UPDATED: Added NEURALCUP support
...
class StrokeFoundationPreprocessor:
    ...
    def preprocess_case(
     ...
     try:
        ...
        # ✅ RESAMPLE MASK TO MATCH IMAGE SPACE IF NEEDED
        if img_nib.shape != mask_nib.shape:
            from nibabel.processing import resample_from_to
            mask_nib_resampled = resample_from_to(mask_nib, img_nib, order=0)
            mask_data = mask_nib_resampled.get_fdata().astype(np.uint8)
        else:
...
def find_image_mask_pairs(data_dir: str, image_pattern: str, mask_pattern: str) -> list:
    ...
    for img_path in image_files:
        ...
        if 'NEURALCUP' in str(data_dir) or 'wmBBS' in img_path.name:
            # ✅ NEURALCUP: wmBBS001_3DT1.nii → BBS001
            identifier = img_path.stem.replace('wm', '').replace('_3DT1', '')
            
            # Find corresponding mask
            mask_candidates = glob(str(data_dir / mask_pattern), recursive=True)
            for mask_candidate in mask_candidates:
                mask_stem = Path(mask_candidate).stem.replace('.nii', '')  # Handle .nii.gz
                if identifier == mask_stem:
                    mask_path = mask_candidate
                    break
        
        elif 'ATLAS' in str(data_dir) or 'sub-r' in img_path.name:
            ...
            case_folder = img_path.parent.parent
            identifier = case_folder.name
