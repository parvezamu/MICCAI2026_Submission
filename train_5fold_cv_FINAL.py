"""
train_5fold_cv_CORRECT.py

Five-fold cross-validation - TRULY FIXED
Loads from BOTH train and val splits, then filters
...
def train_single_fold(fold_idx, train_cases, val_cases, args, exp_dir):
    """Train a single fold - CORRECT APPROACH"""
    ...
    # ✅ CORRECT FIX: Load from BOTH train and val splits, then filter
    print("Loading datasets from BOTH splits...")
    
    # Load train split
    dataset_train_split = PatchDatasetWithCenters(
        ...
        augment=False,  # Don't augment yet, we'll set it after filtering
        lesion_focus_ratio=args.lesion_focus_ratio
    )
    
    # Load val split
    dataset_val_split = PatchDatasetWithCenters(
        preprocessed_dir=args.preprocessed_dir,
        datasets=['ATLAS', 'UOA_Private'],
        split='val',
        splits_file=args.splits_file,
    ...
    # ✅ Combine ALL volumes from both splits
    all_volumes = dataset_train_split.volumes + dataset_val_split.volumes
    
    print(f"✓ Loaded {len(all_volumes)} total volumes from train+val splits\n")
    
    # ✅ Filter to fold-specific cases
    train_cases_set = set(train_cases)
    val_cases_set = set(val_cases)
    
    fold_train_volumes = [v for v in all_volumes if v['case_id'] in train_cases_set]
    fold_val_volumes = [v for v in all_volumes if v['case_id'] in val_cases_set]
    
    print(f"✓ Filtered to fold cases:")
    print(f"  Train: {len(fold_train_volumes)} volumes")
    print(f"  Val:   {len(fold_val_volumes)} volumes\n")
    
    # ✅ Create new dataset instances with filtered volumes
    train_dataset = PatchDatasetWithCenters(
        ...
        patch_size=tuple(args.patch_size),
        patches_per_volume=args.patches_per_volume,
        augment=True,  # ✅ Augment for training
        lesion_focus_ratio=args.lesion_focus_ratio
    )
    train_dataset.volumes = fold_train_volumes  # Override with fold volumes
    ...
    val_dataset = PatchDatasetWithCenters(
        ...
        patch_size=tuple(args.patch_size),
        patches_per_volume=args.patches_per_volume,
        augment=False,  # ✅ No augment for validation
        lesion_focus_ratio=args.lesion_focus_ratio
    )
    val_dataset.volumes = fold_val_volumes  # Override with fold volumes
    ...
    # Verify batch counts
    expected_train_batches = (len(fold_train_volumes) * args.patches_per_volume) // args.batch_size
    expected_val_batches = (len(fold_val_volumes) * args.patches_per_volume + args.batch_size - 1) // args.batch_size
    
    print(f"  Expected train batches: {expected_train_batches}")
    print(f"  Expected val batches: {expected_val_batches}\n")
...
def main():
    ...
    parser.add_argument('--splits-file', type=str, 
                       default='/home/pahm409/ISLES2029/splits_stratified.json')
    ...
    print("\n" + "="*70)
    print("5-FOLD CROSS-VALIDATION - CORRECT VERSION")
    print("="*70)
    print("Loads from BOTH train+val splits, then filters")
    print("="*70 + "\n")
    
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    exp_dir = Path(args.output_dir) / f'5fold_cv_CORRECT_{timestamp}'
