"""
test_neuralcup_1mm_with_printing.py

Test on resampled NEURALCUP (1mm) with DSC printing per case
(UPDATED: deterministic + lesion-focused sparse sampling + brain-mask random sampling)
...
# =========================
# UPDATED NEURALCUP DATASET
# =========================
class NEURALCUPDataset:
    def __init__(self, preprocessed_dir, patch_size=(96,96,96), patches_per_volume=10,
                 lesion_focus_ratio=0.7, seed=42, use_brain_for_random=True):
        self.preprocessed_dir = Path(preprocessed_dir)
        self.patch_size = np.array(patch_size)
        self.patches_per_volume = patches_per_volume
        self.lesion_focus_ratio = lesion_focus_ratio
        self.seed = seed
        self.use_brain_for_random = use_brain_for_random
    ...
    def _sample_center(self, volume, mask, brain_mask, vol_idx, patch_idx):
        vol_shape = np.array(volume.shape)
        half = self.patch_size // 2

        min_center = half
        max_center = vol_shape - half
        ...
        # deterministic per (case, patch)
        rng = np.random.RandomState(self.seed + vol_idx * 1000 + patch_idx)

        prefer_lesion = (rng.rand() < self.lesion_focus_ratio)

        if prefer_lesion:
            lesion_coords = np.where(mask > 0)
            if len(lesion_coords[0]) > 0:
                idx = rng.randint(len(lesion_coords[0]))
                center = np.array([lesion_coords[0][idx],
                                   lesion_coords[1][idx],
                                   lesion_coords[2][idx]], dtype=np.int32)
                center = np.clip(center, min_center, max_center)
                return center

        # random sampling (optionally inside brain)
        if self.use_brain_for_random and brain_mask is not None:
            brain_coords = np.where(brain_mask > 0)
            if len(brain_coords[0]) > 0:
                # pick a brain voxel then clip to valid center range
                idx = rng.randint(len(brain_coords[0]))
                center = np.array([brain_coords[0][idx],
                                   brain_coords[1][idx],
                                   brain_coords[2][idx]], dtype=np.int32)
                center = np.clip(center, min_center, max_center)
                return center

        # fallback: uniform random in valid range
        center = np.array([
            rng.randint(min_center[0], max_center[0] + 1),
            rng.randint(min_center[1], max_center[1] + 1),
            rng.randint(min_center[2], max_center[2] + 1),
        ], dtype=np.int32)
        return center
    ...
    def __getitem__(self, idx):
        ...
        volume = vol['image']
        mask = vol['mask']
        brain_mask = vol['brain_mask']

        center = self._sample_center(volume, mask, brain_mask, vol_idx, patch_idx)
        ...
        # pad if needed (rare if center clipping is correct)
        if patch_image.shape != tuple(self.patch_size):
            ...
            patch_image = np.pad(patch_image, ((0,pad_d),(0,pad_h),(0,pad_w)),
                                 mode='constant', constant_values=volume.min())
            patch_mask  = np.pad(patch_mask,  ((0,pad_d),(0,pad_h),(0,pad_w)),
    ...
    def get_volume_info(self, vol_idx):
        vol = self.volumes[vol_idx]
        return {'case_id': vol['case_id'], 'volume': vol['image'], 'mask': vol['mask']}
...
def reconstruct_from_patches(patch_preds, centers, original_shape, patch_size=(96, 96, 96)):
    ...
    for i, center in enumerate(centers):
        ...
        patch = patch_preds[i, 1, ...]  # class-1 prob
...
def validate_with_reconstruction(model, dataset, device, patch_size):
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(dataset)), desc='Processing patches'):
            ...
            image = batch['image'].unsqueeze(0).to(device)  # (1,1,D,H,W)
            center = batch['center'].numpy()

            output = model(image)
            pred = torch.softmax(output, dim=1).cpu().numpy()  # (1,2,D,H,W)
    ...
    print("="*70)
    print(f"{'Case ID':<15} {'DSC':<10} {'GT Voxels':<12} {'Pred Voxels':<12}")
    print("="*70)
    ...
    for vol_idx in sorted(volume_data.keys()):
        ...
        print(f"{vol_info['case_id']:<15} {dsc:<10.4f} {int(mask_gt.sum()):<12} {int(reconstructed_binary.sum()):<12}")
...
def main():
    ...
    parser.add_argument('--neuralcup-dir', type=str,
                        default='/home/pahm409/preprocessed_NEURALCUP_1mm/NEURALCUP')
    parser.add_argument('--output-dir', type=str,
                        default='/home/pahm409/neuralcup_1mm_results')
    ...
    parser.add_argument('--lesion-focus-ratio', type=float, default=0.7)   # NEW
    parser.add_argument('--seed', type=int, default=42)                    # NEW
    parser.add_argument('--use-brain-for-random', action='store_true')     # NEW
    ...
    print("\n" + "="*70)
    print(f"TESTING {args.fold_name.upper()} ON NEURALCUP (1mm RESAMPLED)")
    print("="*70)
    print("NEURALCUP: sparse reconstruction DSC (lesion-focused + deterministic)")
    print("="*70 + "\n")
    ...
    val_dsc = checkpoint.get('val_dsc_recon', checkpoint.get('val_dsc_recon', 0.0))
    ...
    neuralcup_dataset = NEURALCUPDataset(
        ...
        seed=args.seed,
        use_brain_for_random=args.use_brain_for_random
    )
    ...
    summary = {
        ...
        'sampling': {
            ...
            'lesion_focus_ratio': args.lesion_focus_ratio,
            'seed': args.seed,
            'use_brain_for_random': bool(args.use_brain_for_random)
        }
    }
    ...
    with open(output_dir / 'neuralcup_1mm_results.json', 'w') as f:
        json.dump(summary, f, indent=2)

    print("\n" + "="*70)
    print(f"{args.fold_name.upper()} - NEURALCUP (1mm) TEST RESULTS")
    print("="*70)
    print(f"Cases tested: {len(results)}")
    print(f"Mean DSC:   {summary['neuralcup_1mm_results']['mean_dsc']:.4f} ± {summary['neuralcup_1mm_results']['std_dsc']:.4f}")
    print(f"Median DSC: {summary['neuralcup_1mm_results']['median_dsc']:.4f}")
    print(f"Range:      [{summary['neuralcup_1mm_results']['min_dsc']:.4f}, {summary['neuralcup_1mm_results']['max_dsc']:.4f}]")
    print(f"\nInternal validation DSC: {val_dsc:.4f}")
    print(f"External test DSC:       {summary['neuralcup_1mm_results']['mean_dsc']:.4f}")
    print(f"Generalization gap:      {val_dsc - summary['neuralcup_1mm_results']['mean_dsc']:.4f}")
    print("="*70 + "\n")
    print(f"Results saved to: {output_dir / 'neuralcup_1mm_results.json'}\n")
