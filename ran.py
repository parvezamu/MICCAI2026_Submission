"""
train_segmentation_random_init.py

Segmentation training WITHOUT SimCLR pre-training (Random Initialization)

This script trains the U-Net segmentation model from scratch with randomly
initialized encoder weights, serving as a baseline comparison.

Supports:
- Baseline: Standard U-Net (no attention)
- ECA: Efficient Channel Attention
- MKDC: Multi-Kernel Depthwise Convolution
- Deep Supervision: Auxiliary losses at decoder levels
...
class SegmentationModel(nn.Module):
    """Complete segmentation model: Encoder + Decoder (Random Initialization)"""
...
def main():
    parser = argparse.ArgumentParser(description='Segmentation Training with Random Initialization')
    ...
    parser.add_argument('--ds-weights', type=float, nargs=4, 
                       ...
                       help='Weights for deep supervision')
    parser.add_argument('--encoder-depth', type=int, default=18, choices=[18, 34],
                       help='ResNet encoder depth')
    parser.add_argument('--output-dir', type=str, 
                       default='/home/pahm409/segmentation_random_init_5fold')
    ...
    print("\n" + "="*70)
    print(f"SEGMENTATION TRAINING (RANDOM INIT) - FOLD {args.fold}")
    print(f"Initialization: {init_method}")
    print(f"Encoder: ResNet3D-{args.encoder_depth}")
    ...
    print("="*70)
    print("✓ ROI-based validation")
    print("✓ Robust mask handling (>0)")
    print("✓ NO SimCLR pre-training (random weights)")
    print("="*70 + "\n")
    ...
    if args.encoder_depth == 18:
        encoder = resnet3d_18(in_channels=1)
    elif args.encoder_depth == 34:
        encoder = resnet3d_34(in_channels=1)
    
    print(f"✓ Using ResNet3D-{args.encoder_depth} with RANDOM INITIALIZATION\n")
    ...
    print("="*70)
    print(f"Starting Training - Random Initialization - {attention_names[args.attention]}")
    if args.deep_supervision:
    ...
    for epoch in range(args.epochs):
        ...
        if args.deep_supervision:
            print(f"  [Deep Supervision] [Random Init]")
        else:
            print(f"  [Random Init]")
        print(f"{'='*70}")
