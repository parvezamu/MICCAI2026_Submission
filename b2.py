...
class ConvBN3D(nn.Module):
    """GroupNorm instead of BatchNorm — stable at batch_size=1 or 2"""
...
class SimpleBraTSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Odd patches: lesion-centred sampling (ensures lesion coverage)
        # Even patches: random sampling (background diversity)
        lesion_voxels = np.argwhere(mask > 0)
        use_lesion_centre = (patch_idx % 2 == 0) and (len(lesion_voxels) > 0)

        if use_lesion_centre:
            centre = lesion_voxels[np.random.randint(len(lesion_voxels))]
            d0 = int(np.clip(centre[0] - pd // 2, 0, max(0, D - pd)))
            h0 = int(np.clip(centre[1] - ph // 2, 0, max(0, H - ph)))
            w0 = int(np.clip(centre[2] - pw // 2, 0, max(0, W - pw)))
        else:
