...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, scaler, 
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # ========================================================================
        # FIXED: Use compute_dsc_torch for PyTorch tensors
        # ========================================================================
        with torch.no_grad():
            ...
            dsc = compute_dsc_torch(pred_probs, target_onehot)  # ← FIXED: Use torch version
...
def save_nifti_predictions(model, val_dataset, device, output_dir, epoch, deep_supervision=False, max_volumes=5):
    """
    FIXED: Save NIfTI predictions volume-by-volume (no DataLoader!)
    """
    import nibabel as nib
    
    model.eval()
    
    save_dir = Path(output_dir) / 'nifti_predictions' / f'epoch_{epoch+1:03d}'
    save_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"\n💾 Saving NIfTI predictions to: {save_dir}")
    
    num_volumes = min(max_volumes, len(val_dataset.volumes))
    
    with torch.no_grad():
        for vol_idx in range(num_volumes):
            
            # ================================================================
            # COLLECT ALL PATCHES FOR THIS VOLUME
            # ================================================================
            volume_patches = []
            volume_centers = []
            
            for patch_idx in range(val_dataset.patches_per_volume):
                global_idx = vol_idx * val_dataset.patches_per_volume + patch_idx
                
                if global_idx >= len(val_dataset):
                    break
                
                batch = val_dataset[global_idx]
                
                # Verify it's the right volume
            ...
            reconstructed, count_map = reconstruct_from_patches_with_count(
                all_preds, centers_array, vol_info['mask'].shape, patch_size=(96, 96, 96)
            )
            ...
            # Create affine matrix (MNI space, 1mm isotropic)
            affine = np.array([
                [-1, 0, 0, 90],
                [0, 1, 0, -126],
                [0, 0, 1, -72],
            ...
            # Save NIfTIs
            gt_path = save_dir / f'{case_id}_gt.nii.gz'
            if not gt_path.exists():
                gt_img = nib.Nifti1Image(mask_gt.astype(np.uint8), affine)
                nib.save(gt_img, gt_path)
            
            pred_img = nib.Nifti1Image(pred_binary.astype(np.uint8), affine)
            nib.save(pred_img, save_dir / f'{case_id}_pred.nii.gz')
            
            prob_img = nib.Nifti1Image(reconstructed.astype(np.float32), affine)
            nib.save(prob_img, save_dir / f'{case_id}_prob.nii.gz')
            
            img_path = save_dir / f'{case_id}_image.nii.gz'
            if not img_path.exists():
                image_img = nib.Nifti1Image(vol_info['volume'].astype(np.float32), affine)
                nib.save(image_img, img_path)
            
            print(f"  Saved {case_id}: DSC={dsc:.4f}")
    
    print(f"✓ Saved {num_volumes} volumes\n")
...
def finetune_single_fold(
    ...
    for epoch in range(epochs):
        ...
        # ========================================================================
        # SAVE NIFTI PREDICTIONS EVERY 5 EPOCHS
        # ========================================================================
        if (epoch + 1) % 5 == 0 or epoch == 0:  # Save at epochs 1, 5, 10, 15, etc.
            save_nifti_predictions(
                model=model,
                val_dataset=isles_val,
                device=device,
                output_dir=exp_dir,
                epoch=epoch,
                deep_supervision=deep_supervision,
                max_volumes=5  # Save first 5 validation volumes
            )
