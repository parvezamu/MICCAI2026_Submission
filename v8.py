"""
train_segmentation_unified_v2.py

Enhanced unified segmentation training script with DEEP SUPERVISION and MULTI-RUN support:
...
NEW Features:
- Deep Supervision option for better gradient flow and performance
- Multi-run support with --run-id for repeating experiments
- Random seed support for reproducibility
- Adds auxiliary losses at decoder levels 4, 3, 2
- Minimal overhead (~5-10% slower)
- Expected gain: +0.3-0.5% DSC

All use:
- SimCLR pretrained encoder (optional)
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced Unified Segmentation with Deep Supervision and Multi-Run Support')
    ...
    parser.add_argument('--output-dir', type=str, 
                       default='/home/pahm409/segmentation_unified_v2_5fold')
    ...
    parser.add_argument('--save-nifti-every', type=int, default=50)
    
    args = parser.parse_args()
    
    # Set random seed for reproducibility
    if args.seed is None:
        # Auto-generate seed from fold and run_id if not provided
        base_seed = 42
    ...
    print("✓ Multi-run support with fixed seeds")
    if args.deep_supervision:
        print("✓ Deep supervision for better gradient flow")
    print("="*70 + "\n")
    ...
    if args.run_id is not None:
        exp_dir = Path(args.output_dir) / init_method / f'{args.attention}{ds_suffix}' / f'fold_{args.fold}' / f'run_{args.run_id}' / f'exp_{timestamp}'
    else:
        exp_dir = Path(args.output_dir) / init_method / f'{args.attention}{ds_suffix}' / f'fold_{args.fold}' / f'exp_{timestamp}'
