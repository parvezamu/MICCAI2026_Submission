...
def validate_full_volumes(model, dataloader, device, patch_size, deep_supervision=False,
    ...
    """Process volume-by-volume using existing DataLoader"""
    model.eval()
...
def main():
    parser = argparse.ArgumentParser(description='Ablation Study 5-Fold CV Training')
    ...
    # Seed for reproducibility (fold-specific)
    parser.add_argument('--seed', type=int, default=None,
                       help='Random seed (default: 42 + fold)')
    
    # Data configuration
    parser.add_argument('--output-dir', type=str, 
                       default='/hpc/pahm409/harvard/ablation_atlas_uoa_5fold_cv')
    parser.add_argument('--fold', type=int, required=True, choices=[0, 1, 2, 3, 4])
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch-size', type=int, default=8)
    parser.add_argument('--patch-size', type=int, nargs=3, default=[96, 96, 96])
    parser.add_argument('--patches-per-volume', type=int, default=6)
    ...
    print("\n" + "="*70)
    print(f"ABLATION 5-FOLD CV - FOLD {args.fold}")
    ...
    # Create experiment directory (NO run_id subdirectory)
    ...
    train_dataset = PatchDatasetWithCenters(
        preprocessed_dir='/hpc/pahm409/harvard/preprocessed_stroke_foundation',
    ...
    val_dataset = PatchDatasetWithCenters(
        ...
        fold=args.fold,
        patch_size=tuple(args.patch_size),
        patches_per_volume=10,  # OPTIMIZED: Minimal for fast validation
        augment=False,
    ...
    val_loader = DataLoader(
        val_dataset, 
        batch_size=24,  # OPTIMIZED: Large batch for fast validation
        shuffle=False,
        num_workers=2,  # OPTIMIZED: Increased workers
        pin_memory=True,
        timeout=0
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.validate_recon_every == 0 or epoch == 0:
            ...
            save_nifti = (epoch == args.epochs - 1)
        ...
        print(f"{init_method} - {attention_names[args.attention]}")
