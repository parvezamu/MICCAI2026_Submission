"""
train_patch_improved.py - FINAL FIX

Simple patch-based training with consistent sampling
...
from dataset.patch_dataset_improved import ImprovedPatchDataset
...
def validate(model, dataloader, criterion, device):
    """Validate using same patch sampling as training"""
    model.eval()
...
def plot_training_curves(train_losses, train_dscs, val_losses, val_dscs, save_path):
    ...
    ax.plot(epochs, val_losses, 'r-', linewidth=2, label='Val')
    ...
    ax = axes[0, 1]
    ax.plot(epochs, train_dscs, 'b-', linewidth=2, label='Train')
    ax.plot(epochs, val_dscs, 'r-', linewidth=2, label='Val')
    ax.set_xlabel('Epoch', fontsize=12)
    ax.set_ylabel('DSC', fontsize=12)
    ax.set_title('Dice Score (Patches)', fontsize=14, fontweight='bold')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    ax = axes[1, 0]
    gap = [train_dscs[i] - val_dscs[i] for i in range(len(epochs))]
    ...
    ax = axes[1, 1]
    best_val = [max(val_dscs[:i+1]) for i in range(len(epochs))]
    ax.plot(epochs, best_val, 'g-', linewidth=3, marker='*', markersize=8)
    ax.set_xlabel('Epoch', fontsize=12)
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, 
                       default='/home/pahm409/patch_simple_experiments')
    ...
    print("\n" + "="*70)
    print("SIMPLE PATCH-BASED TRAINING")
    print("="*70)
    print("Same random patch sampling for train AND validation")
    print("="*70 + "\n")
    
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    exp_dir = Path(args.output_dir) / f'patch_simple_{timestamp}'
    ...
    # Setup data - use training mode for BOTH train and val
    print("Setting up patch datasets...")
    train_dataset = ImprovedPatchDataset(
    ...
    # Validation dataset - use 'train' split but with 'val' data
    val_dataset = ImprovedPatchDataset(
        preprocessed_dir='/home/pahm409/preprocessed_stroke_foundation',
        datasets=['ATLAS', 'UOA_Private'],
        split='val',  # This will load val data but use train __getitem__ logic
        splits_file='splits_stratified.json',
        patch_size=tuple(args.patch_size),
        patches_per_volume=args.patches_per_volume,
    ...
    # Force validation dataset to use train mode (random patches)
    ...
    for epoch in range(args.epochs):
        ...
        print(f"  Gap: {train_dsc - val_dsc:+.4f}  |  LR: {current_lr:.6f}")
