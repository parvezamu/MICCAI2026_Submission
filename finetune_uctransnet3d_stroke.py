...
# ============================================================
# SLIDING WINDOW VOLUME VALIDATION
# ============================================================
def sliding_window_inference(model, volume, device, patch_size, step_size=0.5, batch_size=4):
    model.eval()
    D, H, W    = volume.shape
    pd, ph, pw = patch_size

    def starts(dim, p):
        s  = max(1, int(p * step_size))
        st = list(range(0, dim - p + 1, s))
        if not st or st[-1] + p < dim:
            st.append(max(0, dim - p))
        return st

    coords     = [(d,h,w) for d in starts(D,pd) for h in starts(H,ph) for w in starts(W,pw)]
    ...
    with torch.no_grad():
        for i in range(0, len(coords), batch_size):
            ...
            patches = [volume[d:d+pd, h:h+ph, w:w+pw] for d,h,w in bc]
            tensor  = torch.from_numpy(np.stack(patches)).unsqueeze(1).float().to(device)
            preds   = torch.softmax(model(tensor).float(), dim=1)[:,1].cpu().numpy()
            for j, (d,h,w) in enumerate(bc):
                prediction[d:d+pd, h:h+ph, w:w+pw] += preds[j]
                count_map [d:d+pd, h:h+ph, w:w+pw] += 1.0

    valid = count_map > 0
    prediction[valid] /= count_map[valid]
    return prediction


def validate_volumes(model, val_ids, preprocessed_dir, dataset_name,
                     device, patch_size, max_cases=None, save_dir=None):
    model.eval()
    ids  = val_ids[:max_cases] if max_cases else val_ids
    dscs = []

    for case_id in ids:
        npz = Path(preprocessed_dir) / dataset_name / f"{case_id}.npz"
        if not npz.exists():
            print(f"    SKIP {case_id}: not found"); continue
        try:
            data = np.load(npz)
            vol  = data['image'].astype(np.float32)
            mask = data['lesion_mask'] if 'lesion_mask' in data.files else data['mask']
            gt   = (mask > 0).astype(np.uint8)

            pred_prob   = sliding_window_inference(model, vol, device, patch_size)
            ...
            dscs.append(dsc)
            print(f"    {case_id}: DSC={dsc:.4f}  GT={gt.sum()}  Pred={pred_binary.sum()}")

            if save_dir and HAS_NIBABEL:
                sd = Path(save_dir) / case_id
                sd.mkdir(parents=True, exist_ok=True)
                aff = np.eye(4)
                nib.save(nib.Nifti1Image(pred_binary, aff), sd / 'pred.nii.gz')
                nib.save(nib.Nifti1Image(pred_prob,   aff), sd / 'pred_prob.nii.gz')
                nib.save(nib.Nifti1Image(gt,          aff), sd / 'gt.nii.gz')
...
def run_experiment(dataset_name, fold, use_transfer, cfg):
    ...
    for epoch in range(cfg['epochs']):
        ...
        if epoch == 0 or ep % 10 == 0:
            ...
            vol_dsc, _ = validate_volumes(model, val_ids, cfg['preprocessed_dir'],
                                          dataset_name, device, ps,
