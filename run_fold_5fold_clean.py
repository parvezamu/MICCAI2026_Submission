...
OUTPUT_DIR = "/home/pahm409/dwi_isles_5fold_cv"  # Updated directory name
GPU_ID = "0"
...
def get_config_name(use_mkdc, deep_supervision):
    """Derive config name from flags (matches training script logic)"""
...
def get_experiment_status(base_dir, config_name, fold):
    """Check if experiment is completed"""
    fold_dir = Path(base_dir) / config_name / f'fold_{fold}'
...
def run_single_experiment(exp_config, fold, output_dir, gpu_id, resume_checkpoint=None):
    ...
    cmd = [
        'python', 'train_dwi_scratch_5fold_clean.py',
        '--fold', str(fold),
        '--epochs', str(EPOCHS),
...
def main():
    parser = argparse.ArgumentParser(description='Train all experiments for a specific fold (DWI 5-Fold CV)')
    ...
    print("\n" + "="*80)
    print(f"DWI 5-FOLD CV - FOLD {fold} TRAINING")
    print("="*80)
