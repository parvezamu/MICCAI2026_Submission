"""
Ablation Study - Part 1: WITH Pre-training

Train segmentation model WITH SimCLR pre-trained initialization
GPU: 4
...
def train_epoch(model, dataloader, criterion, optimizer, device, scaler, epoch, total_epochs):
    ...
    pbar = tqdm(dataloader, desc=f'[PRETRAINED] Epoch {epoch+1}/{total_epochs}')
...
def validate_epoch(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='[PRETRAINED] Val (patches)'):
            images = batch['image'].to(device)
            masks = batch['mask'].to(device)
...
def validate_with_reconstruction(model, dataset, device, patch_size=(96, 96, 96)):
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(dataset)), desc='[PRETRAINED] Processing'):
            batch = dataset[idx]
...
def main():
    ...
    print("\n" + "="*70)
    print("ABLATION STUDY - PART 1: WITH PRE-TRAINING")
    print("="*70)
    ...
    # SimCLR was trained on full volumes, but we're using patches
    # This causes BatchNorm stats mismatch and poor initial performance
    print("\n  Resetting BatchNorm running statistics...")
    for module in encoder.modules():
        if isinstance(module, nn.BatchNorm3d):
            module.reset_running_stats()
    print("  ✓ BatchNorm stats reset for patch-based training\n")
    ...
    for epoch in range(args.epochs):
        ...
        # Print summary
        print(f"\n{'='*70}")
        print(f"[PRETRAINED] Epoch {epoch+1}/{args.epochs}")
        print(f"{'='*70}")
    ...
    axes[0, 0].set_title('DSC - Pre-trained Init', fontweight='bold')
    axes[0, 0].legend()
    ...
    axes[0, 1].set_title('Loss - Pre-trained Init', fontweight='bold')
    axes[0, 1].legend()
    ...
    print("\n" + "="*70)
    print("PRE-TRAINED TRAINING COMPLETE!")
    print("="*70)
