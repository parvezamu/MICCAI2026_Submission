"""
joint_training_SEPARATE_LR_FINAL_FROZEN.py

FINAL VERSION with:
- Geometrically aligned augmentations
- Correct Generalized Dice Loss
- OPTION TO FREEZE DWI ENCODER (fixes DSC=0 problem)
"""
...
class JointTrainingModels(nn.Module):
    def __init__(self, pretrained_encoder_path=None, num_classes=2, freeze_dwi_encoder=True):
        super().__init__()
        ...
        # FREEZE DWI ENCODER if requested
        self.freeze_dwi_encoder = freeze_dwi_encoder
        if freeze_dwi_encoder and pretrained_encoder_path:
            for param in self.encoder_dwi.parameters():
                param.requires_grad = False
            print("🔒 DWI encoder FROZEN (decoder will learn to use pretrained features)")
...
def main():
    ...
    parser.add_argument('--output-dir', type=str,
                       default='/home/pahm409/joint_training_final_frozen')
    parser.add_argument('--fold', type=int, default=0)
    parser.add_argument('--freeze-dwi-encoder', action='store_true', default=True,
                       help='Freeze DWI encoder (recommended)')
    parser.add_argument('--no-freeze-dwi-encoder', dest='freeze_dwi_encoder', 
                       action='store_false',
                       help='Train DWI encoder (not recommended if pretrained)')
    ...
    # Learning rates
    if args.freeze_dwi_encoder:
        # Only decoder learns, can use higher LR
        DWI_LR = 0.001  # Only decoder
        ADC_LR = 0.001
    else:
        # Encoder + decoder learn, use different LRs
        DWI_LR = 0.0001  # Pretrained encoder learns slowly
        ADC_LR = 0.001   # Random init needs higher LR
    ...
    print(f"DWI encoder: {'FROZEN' if args.freeze_dwi_encoder else 'TRAINABLE'}")
    print(f"DWI LR: {DWI_LR}")
    print(f"ADC LR: {ADC_LR}")
    ...
    model = JointTrainingModels(
        ...
        num_classes=2,
        freeze_dwi_encoder=args.freeze_dwi_encoder
    ).to(device)
    ...
    dwi_params = [p for p in list(model.encoder_dwi.parameters()) + list(model.decoder_dwi.parameters()) 
                  if p.requires_grad]
    ...
    dwi_trainable = sum(p.numel() for p in dwi_params)
    adc_trainable = sum(p.numel() for p in adc_params)
    
    print("✓ Optimizer created:")
    print(f"  DWI trainable: {dwi_trainable:,} params at LR={DWI_LR}")
    print(f"  ADC trainable: {adc_trainable:,} params at LR={ADC_LR}")
    if args.freeze_dwi_encoder:
        encoder_params = sum(p.numel() for p in model.encoder_dwi.parameters())
        print(f"  DWI encoder frozen: {encoder_params:,} params")
    print()
    
    # CORRECT: Generalized Dice Loss with softmax
    criterion = GDiceLoss(apply_nonlin=torch.softmax, smooth=1e-5)
        ...
        if val_dsc > best_dsc:
            ...
            torch.save({
                ...
                'config': {
                    ...
                    'adc_lr': ADC_LR,
                    'loss': 'GDiceLoss',
                    'dwi_encoder_frozen': args.freeze_dwi_encoder,
