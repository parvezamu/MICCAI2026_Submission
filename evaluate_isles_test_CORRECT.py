...
def get_original_isles_info(case_id, isles_raw_dir='/home/pahm409/ISLES2022_reg/ISLES2022'):
    ...
    # ISLES structure: ISLES2022/sub-strokecaseXXXX/sub-strokecaseXXXX_ses-0001_dwi.nii.gz
...
def load_model_from_checkpoint(checkpoint_path, device):
    ...
    return model, attention_type, deep_supervision, fold
...
def evaluate_test_set(model, test_dataset, device, patch_size=(96, 96, 96), 
                     ...
                     isles_raw_dir='/home/pahm409/ISLES2022_reg/ISLES2022'):
    ...
    for vol_idx in tqdm(sorted(volume_data.keys()), desc='Reconstructing'):
        ...
        # Get ground truth from original files
        ...
        if vol_idx < 5:
            ...
            print(f"    Pred volume: {pred_binary_original.sum()} voxels")
            print(f"    DSC: {dsc_original:.4f}")
        
        # Save NIfTI in original space
...
def evaluate_single_checkpoint(
    ...
    # Load model
    model, attention_type, deep_supervision, fold = load_model_from_checkpoint(
    ...
    print(f"Test dataset: {len(test_dataset.volumes)} volumes")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate fine-tuned ISLES model on test set')
    
    parser.add_argument('--checkpoint', type=str, required=True,
                       help='Path to checkpoint file (best_finetuned_model.pth)')
