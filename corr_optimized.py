"""
train_segmentation_corrected.py

CORRECTED unified segmentation training script with:
- PROPER learning rate scaling for large batch sizes
- Linear warmup for stable training
- AdamW optimizer with weight decay
- Improved learning rate scheduling
- All previous features (Deep Supervision, Multi-Run support, MKDC, etc.)
- **NEW: Resume training from checkpoint**

KEY CHANGES from train_segmentation_unified_v2.py:
1. LR scaling: batch_size 8→32 requires lr 0.0001→0.0004 (4× scaling)
2. Warmup: 5 epochs linear warmup for large batch stability
3. AdamW: Better than Adam for large batch + weight decay
4. Cosine annealing with warmup
5. Gradient clipping for stability
6. **Resume support: --resume-checkpoint to continue from saved state**

Author: Parvez (Corrected + Resume)
Date: January 2026
"""
...
def compute_deep_supervised_loss(outputs, target, criterion, weights=[1.0, 0.5, 0.25, 0.125]):
    ...
    Args:
        outputs: List of [main_output, ds4, ds3, ds2]
        target: Ground truth mask
        criterion: Loss function
        weights: Weights for [main, ds4, ds3, ds2]
    """
    total_loss = 0.0
...
class WarmupCosineScheduler:
    ...
    This is CRITICAL for large batch training stability!
    """
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, scaler, 
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with autocast():
            ...
            if deep_supervision:
                # outputs is a list [main, ds4, ds3, ds2]
                loss = compute_deep_supervised_loss(outputs, masks, criterion, ds_weights)
                main_output = outputs[0]  # Use main output for DSC
            else:
        ...
        # Gradient clipping for stability (important for large batch!)
        ...
        with torch.no_grad():
            # Compute DSC on main output only
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='Validating (patches)'):
            ...
            with autocast():
                ...
                # Only use main output loss for validation
...
def validate_full_volumes(model, dataloader, device, patch_size, deep_supervision=False,
                         save_nifti=False, save_dir=None, epoch=None):
    """
    FIXED: Process volume-by-volume using EXISTING DataLoader
    This prevents hanging caused by creating new DataLoader
    Works for both ATLAS/UOA and ISLES datasets
    """
    from collections import defaultdict
    import time
    ...
    # Get dataset from existing dataloader
    ...
    print(f"\n[DEBUG] Starting full volume reconstruction at {time.strftime('%H:%M:%S')}")
    print(f"Reconstructing {num_volumes} volumes...")
    
    # Use EXISTING dataloader passed as parameter - no new DataLoader creation!
    ...
    print(f"[DEBUG] Finished patch processing at {time.strftime('%H:%M:%S')}")
    ...
    print(f"[DEBUG] Finished reconstruction at {time.strftime('%H:%M:%S')}")
...
def compute_effective_lr(batch_size, base_batch_size=8, base_lr=0.0001):
    ...
    When batch size increases, gradient averaging becomes more stable,
    so we can use a proportionally larger learning rate.
    
    Formula: new_lr = base_lr × (new_batch_size / base_batch_size)
    """
...
def load_checkpoint_for_resume(checkpoint_path, model, optimizer, scheduler, device):
    ...
    Returns:
        ...
        best_dsc: Best DSC so far
        train_losses, train_dscs, val_dscs_patch, val_dscs_recon: Training history
    """
    print(f"\n{'='*70}")
...
def main():
    parser = argparse.ArgumentParser(description='CORRECTED Unified Segmentation with Resume Support')
    ...
    parser.add_argument('--deep-supervision', action='store_true',
                       help='Enable deep supervision (auxiliary losses at decoder levels)')
    ...
    parser.add_argument('--seed', type=int, default=None,
                       help='Random seed for reproducibility (auto-computed if not provided)')
    
    # Data configuration
    parser.add_argument('--output-dir', type=str, 
                       default='/home/pahm409/segmentation_corrected_5fold')
    ...
    # Learning rate with automatic scaling
    parser.add_argument('--lr', type=float, default=None,
                       help='Learning rate (if None, auto-computed based on batch size)')
    parser.add_argument('--base-lr', type=float, default=0.0001,
                       help='Base learning rate for batch_size=8')
    ...
    if args.lr is None:
        ...
        print(f"\n⚠️  Using manually specified LR: {args.lr:.6f}")
        print(f"   Recommended LR for batch_size={args.batch_size}: "
              f"{compute_effective_lr(args.batch_size, args.base_batch_size, args.base_lr):.6f}\n")
    ...
    print("\n" + "="*70)
    print(f"CORRECTED SEGMENTATION TRAINING - FOLD {args.fold}")
    if args.run_id is not None:
    ...
    if args.resume_checkpoint:
        print(f"Resume: YES (from {args.resume_checkpoint})")
    else:
        print(f"Resume: NO (training from scratch)")
    print("="*70)
    print(f"✓ Batch size: {args.batch_size}")
    print(f"✓ Learning rate: {args.lr:.6f} (scaled for batch size)")
    ...
    # Create experiment directory with run_id if specified
    if args.run_id is not None:
        exp_dir = Path(args.output_dir) / init_method / f'{args.attention}{ds_suffix}' / \
                  f'fold_{args.fold}' / f'run_{args.run_id}' / f'exp_{timestamp}'
    else:
        exp_dir = Path(args.output_dir) / init_method / f'{args.attention}{ds_suffix}' / \
                  f'fold_{args.fold}' / f'exp_{timestamp}'
    ...
    # Training: Use multiple workers for speed (persistent workers stay alive between epochs)
    train_loader = DataLoader(
        train_dataset, 
        batch_size=args.batch_size, 
        shuffle=True,
        num_workers=4,              # Multiple workers for parallel data loading
        ...
        pin_memory=True, 
        drop_last=True,
        timeout=0                    # Disable timeout to prevent hanging
    )
    
    # Validation: Use single worker to avoid hanging issues during validation
    val_loader = DataLoader(
        val_dataset, 
        batch_size=args.batch_size, 
        shuffle=False,
        num_workers=1,               # Single worker (or 0 if hanging persists)
        pin_memory=True,
    ...
    if args.optimizer == 'adamw':
        ...
        print(f"✓ Optimizer: AdamW (lr={args.lr:.6f}, weight_decay={args.weight_decay})")
        ...
        print(f"✓ Optimizer: Adam (lr={args.lr:.6f}, weight_decay={args.weight_decay})")
    else:  # sgd
        optimizer = optim.SGD(
            model.parameters(),
            lr=args.lr,
        ...
        print(f"✓ Optimizer: SGD+Nesterov (lr={args.lr:.6f}, momentum=0.9)")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.validate_recon_every == 0 or epoch == 0:
            ...
            save_nifti = (epoch == args.epochs - 1)  # Only save at final epoch (epoch 99)
            val_dsc_recon, all_dscs = validate_full_volumes(
                model, val_loader, device, tuple(args.patch_size),
                deep_supervision=args.deep_supervision,
        ...
        if val_dsc_recon > best_dsc:
            ...
            # **UPDATED: Save more complete checkpoint for resuming**
            torch.save({
                'epoch': epoch,
                'fold': args.fold,
                'run_id': args.run_id,
    ...
    with open(exp_dir / 'final_summary.json', 'w') as f:
        json.dump({
            ...
            'warmup_epochs': args.warmup_epochs,
            'weight_decay': args.weight_decay,
            'resumed_from': args.resume_checkpoint if args.resume_checkpoint else None
        }, f, indent=4)
