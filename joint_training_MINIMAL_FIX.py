...
Minimal fixes to your working code:
1. Fixed rotation - generate angle ONCE for dwi, adc, mask
2. Fixed elastic - generate displacement field ONCE
3. Fixed scaling - apply same scale to all
4. Keep your working loss function (Soft Dice)
5. Keep mode='constant' (your original approach)
"""
...
# ============================================================================
# AUGMENTATION FUNCTIONS - MINIMALLY FIXED
# ============================================================================

def apply_augmentations_fixed(dwi, adc, mask):
    """
    Fixed version: spatial transforms use SAME parameters for dwi, adc, mask
    Intensity transforms can differ (they don't affect spatial alignment)
    """
    
    # 1. ROTATION - generate angle ONCE, apply to all
    ...
    # 2. ELASTIC DEFORMATION - generate displacement field ONCE, apply to all
    if np.random.rand() > 0.7:
        alpha = 10
        sigma = 3
        shape = dwi.shape
        
        # Generate displacement ONCE
    ...
    # 3. SCALING - generate scale ONCE, apply to all
    if np.random.rand() > 0.5:
    ...
    # 4. FLIPPING - generate axis ONCE, apply to all
    if np.random.rand() > 0.5:
    ...
    # ========================================================================
    # INTENSITY AUGMENTATIONS - Can differ between modalities
    # ========================================================================
...
# ============================================================================
# LOSS - YOUR WORKING SOFT DICE
# ============================================================================

class GDiceLossV2(nn.Module):
    """
    Soft Dice Loss (keeping your original that works)
    """
    ...
    def forward(self, pred, target):
        ...
        # Keep batch dimension separate [B, C, D*H*W]
...
def main():
    ...
    parser.add_argument('--output-dir', type=str,
                       default='/home/pahm409/joint_training_minimal_fix')
    parser.add_argument('--fold', type=int, default=0)
    args = parser.parse_args()
    
    # Settings - SAME AS YOUR WORKING CODE
    EPOCHS = 200
    BATCH_SIZE = 8
    PATCH_SIZE = (96, 96, 96)
    DWI_LR = 0.0001
    ...
    print("="*80)
    print("JOINT TRAINING - MINIMAL FIX (ALIGNED AUGMENTATIONS)")
    print("="*80)
    print(f"Loss: Soft Dice (your working version)")
        ...
        if val_dsc > best_dsc:
            ...
            torch.save({
                ...
                'config': {
                    ...
                    'dwi_lr': DWI_LR,
                    'adc_lr': ADC_LR,
                    'augmentation': 'aligned_minimal_fix'
                }
