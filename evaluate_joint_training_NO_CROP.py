#!/usr/bin/env python3
"""
evaluate_joint_training_NO_CROP.py

Evaluation without cropping - direct resample 96³ ↔ original
"""
...
def resample_to_original(pred_96, original_shape, method='linear'):
    """Direct resample from 96³ to original - NO cropping"""
    zoom_factors = np.array(original_shape, dtype=np.float32) / np.array([96, 96, 96], dtype=np.float32)
    order = 0 if method == 'nearest' else 1
    
    resampled = zoom(pred_96.astype(np.float32), zoom_factors, order=order)
    
    # Ensure exact size
    if resampled.shape != original_shape:
        fixed = np.zeros(original_shape, dtype=resampled.dtype)
        slices = tuple(slice(0, min(resampled.shape[i], original_shape[i])) for i in range(3))
...
def evaluate_test_set(model, test_cases, npz_dir, device, save_nifti=False, 
    ...
    """Evaluate without cropping"""
    import nibabel as nib
    
    model.eval()
    
    print(f"\n{'='*80}")
    print(f"TEST SET EVALUATION - NO CROPPING")
    print(f"{'='*80}")
    ...
    print(f"{'Case ID':<25} {'DSC':<8} {'GT':<8} {'Pred':<8} {'Original Shape'}")
    print("-" * 80)
    ...
    for case_id in tqdm(test_cases, desc='Evaluating'):
        ...
        # Resample to original - NO cropping
        pred_prob_original = resample_to_original(pred_prob_96, original_shape, method='linear')
        ...
        print(f"{case_id:<25} {dsc:<8.4f} {gt_vol:<8} {pred_vol:<8} {original_shape}")
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"JOINT TRAINING EVALUATION - NO CROPPING")
    print(f"{'='*80}")
    ...
    output_dir = Path(args.output_dir) / f'fold_{fold}_NO_CROP'
