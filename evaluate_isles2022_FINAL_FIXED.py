"""
evaluate_isles2022_FINAL_FIXED.py

Evaluate on FIXED ISLES preprocessing with correct lesion contrast
"""
...
class ISLESDatasetForEval(Dataset):
    """ISLES evaluation dataset with lesion-focused sampling"""
    ...
    def __init__(self, isles_dir, patch_size=(96, 96, 96), patches_per_volume=100, lesion_focus_ratio=0.7):
        ...
        print(f"✓ Loaded {len(self.volumes)} ISLES cases (FIXED preprocessing)")
        print(f"  Patches per volume: {patches_per_volume}")
        print(f"  Lesion focus ratio: {lesion_focus_ratio}")
    ...
    def _extract_patch_with_center(self, volume, mask):
        """Extract patch with lesion-focused sampling (70%)"""
...
def evaluate_isles(model_checkpoint, isles_dir, output_dir, 
    ...
    """Evaluate on FIXED ISLES preprocessing"""
    ...
    print("="*95)
    print(f"{'CaseID':<30} {'DSC':>8} {'GT':>10} {'Pred':>10} {'Inter':>10} {'Patches':>8}")
    print("="*95)
    ...
    for vol_idx in tqdm(sorted(volume_data.keys()), desc='Reconstructing volumes'):
        ...
        print(f"{case_id:<30} {result['dsc']:>8.4f} {result['lesion_gt']:>10} "
    ...
    small = [r for r in results if r['lesion_gt'] < 1000]
    medium = [r for r in results if 1000 <= r['lesion_gt'] < 10000]
    large = [r for r in results if r['lesion_gt'] >= 10000]
    
    # Save results
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    
    with open(output_path / 'isles2022_results_FIXED.txt', 'w') as f:
        f.write("="*80 + "\n")
        f.write("ISLES 2022 External Validation Results (FIXED Preprocessing)\n")
        f.write("="*80 + "\n\n")
        
        f.write(f"Model: {model_checkpoint}\n")
        f.write(f"Cases: {len(results)}\n")
        f.write(f"Preprocessing: FIXED (auto-inverted to match T1 contrast)\n\n")
        
        f.write("Overall Statistics:\n")
        f.write(f"  Mean DSC:   {stats['mean']:.4f} ± {stats['std']:.4f}\n")
        f.write(f"  Median DSC: {stats['median']:.4f}\n")
        f.write(f"  Q1-Q3:      [{stats['q25']:.4f}, {stats['q75']:.4f}]\n")
        f.write(f"  Range:      [{stats['min']:.4f}, {stats['max']:.4f}]\n\n")
        
        if small:
            f.write(f"Small lesions (<1000 voxels, n={len(small)}):\n")
            f.write(f"  Mean DSC: {np.mean([r['dsc'] for r in small]):.4f}\n\n")
        if medium:
            f.write(f"Medium lesions (1000-10000 voxels, n={len(medium)}):\n")
            f.write(f"  Mean DSC: {np.mean([r['dsc'] for r in medium]):.4f}\n\n")
        if large:
            f.write(f"Large lesions (>10000 voxels, n={len(large)}):\n")
            f.write(f"  Mean DSC: {np.mean([r['dsc'] for r in large]):.4f}\n\n")
        
        f.write("="*80 + "\n")
        f.write("Per-Case Results (sorted by DSC):\n")
        f.write("="*80 + "\n")
        f.write(f"{'CaseID':<30} {'DSC':>8} {'GT':>10} {'Pred':>10}\n")
        f.write("-"*80 + "\n")
        
        for r in sorted(results, key=lambda x: x['dsc'], reverse=True):
            f.write(f"{r['case_id']:<30} {r['dsc']:>8.4f} {r['lesion_gt']:>10} {r['lesion_pred']:>10}\n")
    
    print("\n" + "="*95)
    print("EVALUATION COMPLETE - ISLES 2022 (FIXED PREPROCESSING)")
    print("="*95)
    print(f"Cases:      {len(results)}")
    print(f"Mean DSC:   {stats['mean']:.4f} ± {stats['std']:.4f}")
    print(f"Median DSC: {stats['median']:.4f}")
    print(f"Range:      [{stats['min']:.4f}, {stats['max']:.4f}]")
    
    if small:
        print(f"\nSmall lesions (n={len(small)}):  {np.mean([r['dsc'] for r in small]):.4f}")
    if medium:
        print(f"Medium lesions (n={len(medium)}): {np.mean([r['dsc'] for r in medium]):.4f}")
    if large:
        print(f"Large lesions (n={len(large)}):  {np.mean([r['dsc'] for r in large]):.4f}")
    
    print("="*95)
    print(f"\nResults saved: {output_path / 'isles2022_results_FIXED.txt'}")
...
if __name__ == '__main__':
    ...
    # Run evaluation on FIXED preprocessing
    results, stats = evaluate_isles(
        model_checkpoint=paths[0],
        isles_dir="/home/pahm409/isles2022_preprocessed_FIXED",  # ← FIXED directory
        output_dir="/home/pahm409/isles2022_evaluation_FIXED",
        patch_size=(96, 96, 96),
