#!/usr/bin/env python3
"""
evaluate_joint_training_FINAL_FIXED.py

FIXED: Use order=0 (nearest neighbor) for resampling to preserve predictions
"""
...
def reconstruct_from_patches_with_count(patch_preds, centers, original_shape, patch_size=(96, 96, 96)):
    """Reconstruct volume from patches with proper bounds checking"""
...
# ============================================================================
# REVERSE PREPROCESSING WITH BBOX - FIXED!
# ============================================================================

def reverse_preprocessing_fixed(pred_96, metadata):
    """
    FIXED: Reverse preprocessing with order=0 for better preservation
    
    Steps:
    1. Resample from 96³ to cropped size (use order=0 for thresholded predictions)
    ...
    if not bbox_valid:
        # No cropping - direct resample
        zoom_factors = np.array(original_shape, dtype=np.float32) / np.array([96, 96, 96], dtype=np.float32)
        pred_original = zoom(pred_96.astype(np.float32), zoom_factors, order=0)  # ← order=0
    ...
    # STEP 1: Threshold first in 96³ space
    pred_96_binary = (pred_96 > 0.5).astype(np.float32)
    
    # STEP 2: Resample to cropped size with nearest neighbor
    zoom_factors = np.array(cropped_shape, dtype=np.float32) / np.array([96, 96, 96], dtype=np.float32)
    pred_cropped = zoom(pred_96_binary, zoom_factors, order=0)  # ← CRITICAL: order=0!
    ...
    # STEP 3: Uncrop to original size
...
def evaluate_test_set(model, test_dataset, device, patch_size=(96, 96, 96),
    ...
    print(f"\n{'='*80}")
    print(f"TEST SET EVALUATION - FIXED RESAMPLING")
    print(f"{'='*80}")
    ...
    for vol_idx in tqdm(sorted(volume_data.keys()), desc='Reconstructing'):
        try:
            ...
            # STEP 3: Reverse preprocessing with FIXED order=0
            pred_binary_original = reverse_preprocessing_fixed(reconstructed_96, metadata).astype(np.uint8)
            ...
            print(f"{case_id:<25} {dsc:<8.4f} {gt_vol:<8} {pred_vol:<8} {coverage:<10.1f} {metadata['original_shape']}")
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"JOINT TRAINING EVALUATION - FIXED")
    print(f"{'='*80}")
    ...
    output_dir = Path(args.output_dir) / f'fold_{fold}_FIXED'
