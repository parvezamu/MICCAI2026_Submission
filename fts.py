...
def load_pretrained_model(checkpoint_path, device):
    ...
    # Check what keys are in model_state_dict
    state_dict = checkpoint.get('model_state_dict', {})
    
    # Detect SimCLR: has projection_head but no decoder
    ...
    if is_simclr:
        # SimCLR checkpoint - extract only encoder weights
        print(f"\n✓ Detected SimCLR checkpoint")
        ...
        # Extract ONLY encoder weights from model_state_dict
        encoder_state_dict = {}
        for key, value in state_dict.items():
            if key.startswith('encoder.'):
                # Remove 'encoder.' prefix to match ResNet3D structure
        ...
        # Create segmentation model with pre-trained encoder + random decoder
        ...
        print(f"  SimCLR epoch: {checkpoint.get('epoch', 'N/A')}")
        print(f"  Loss: {checkpoint.get('loss', 'N/A'):.4f}" if 'loss' in checkpoint else "  Loss: N/A")
        print(f"  ✓ Encoder loaded from SimCLR (BraTS pre-training)")
        print(f"  ⚠️  Decoder randomly initialized (will learn during fine-tuning)")
        
    else:
        # Supervised checkpoint - full model is pre-trained
        print(f"\n✓ Detected supervised checkpoint")
