...
def validate_full_volumes(model, dataset, device, patch_size, deep_supervision=False,
    ...
    """
    OPTIMIZED: Process volume-by-volume with proper batch processing
...
def main():
    parser = argparse.ArgumentParser(description='CORRECTED Unified Segmentation with Proper LR Scaling')
    ...
    # CORRECTED: Learning rate with automatic scaling
    ...
    # NEW: Optimizer and scheduler options
    ...
    # CRITICAL: Compute effective learning rate using linear scaling rule
    if args.lr is None:
    ...
    print(f"✓ ROI-based validation (fair comparison)")
    print(f"✓ Robust mask handling (>0)")
    print(f"✓ Fixed: No double softmax bug")
    print(f"✓ Multi-run support with fixed seeds")
    if args.deep_supervision:
        print(f"✓ Deep supervision for better gradient flow")
    print("="*70 + "\n")
    ...
    # CORRECTED: Use AdamW with weight decay (better than Adam for large batch)
    ...
    # CORRECTED: Use warmup + cosine annealing scheduler
    ...
    print("\n" + "="*70)
    print(f"Starting CORRECTED Training - {init_method} - {attention_names[args.attention]}")
    if args.run_id is not None:
