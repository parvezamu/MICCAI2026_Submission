...
Joint training WITHOUT transfer learning:
- Two separate models (DWI and ADC)
- Both trained from random initialization
...
sys.path.append('/hpc/pahm409/experiments')
...
def apply_augmentations_fixed(dwi, adc, mask):
    """Spatial transforms use SAME parameters for dwi, adc, mask"""
    ...
    if np.random.rand() > 0.7:
        ...
        dwi = map_coordinates(dwi, indices, order=1, mode='reflect').reshape(shape)
        adc = map_coordinates(adc, indices, order=1, mode='reflect').reshape(shape)
    ...
    if np.random.rand() > 0.5:
        ...
        dwi_scaled = zoom(dwi, scale, order=1, mode='constant', cval=0)
        adc_scaled = zoom(adc, scale, order=1, mode='constant', cval=0)
        mask_scaled = zoom(mask, scale, order=0, mode='constant', cval=0)
        
        original_shape = dwi.shape
        current_shape = dwi_scaled.shape
        diff = [c - o for c, o in zip(current_shape, original_shape)]
        
        if all(d >= 0 for d in diff):
            start = [d // 2 for d in diff]
            dwi = dwi_scaled[start[0]:start[0]+original_shape[0],
                            start[1]:start[1]+original_shape[1],
                            start[2]:start[2]+original_shape[2]]
            adc = adc_scaled[start[0]:start[0]+original_shape[0],
                            start[1]:start[1]+original_shape[1],
                            start[2]:start[2]+original_shape[2]]
            ...
            dwi = np.pad(dwi_scaled, pad_amounts, mode='constant', constant_values=0)
            adc = np.pad(adc_scaled, pad_amounts, mode='constant', constant_values=0)
            mask = np.pad(mask_scaled, pad_amounts, mode='constant', constant_values=0)
    ...
    # INTENSITY AUGMENTATIONS (can differ)
    if np.random.rand() > 0.5:
        dwi = gaussian_filter(dwi, sigma=0.5)
    if np.random.rand() > 0.5:
        adc = gaussian_filter(adc, sigma=0.5)
    
    if np.random.rand() > 0.5:
        dwi = dwi + np.random.normal(0, 0.01, dwi.shape)
    if np.random.rand() > 0.5:
        adc = adc + np.random.normal(0, 0.01, adc.shape)
    
    if np.random.rand() > 0.5:
        dwi = dwi * np.random.uniform(0.8, 1.2)
    if np.random.rand() > 0.5:
        adc = adc * np.random.uniform(0.8, 1.2)
    
    if np.random.rand() > 0.5:
        mean = dwi.mean()
        dwi = (dwi - mean) * np.random.uniform(0.8, 1.2) + mean
    if np.random.rand() > 0.5:
        mean = adc.mean()
        adc = (adc - mean) * np.random.uniform(0.8, 1.2) + mean
    ...
    if np.random.rand() > 0.5:
        ...
        if dwi_max > dwi_min:
            ...
            dwi = np.power(dwi_norm, gamma) * (dwi_max - dwi_min) + dwi_min
    ...
    if np.random.rand() > 0.5:
        ...
        if adc_max > adc_min:
            ...
            adc = np.power(adc_norm, gamma) * (adc_max - adc_min) + adc_min
...
class DualModalityDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            dwi_patch, adc_patch, mask_patch = apply_augmentations_fixed(
...
def main():
    ...
    parser.add_argument('--output-dir', type=str,
                       default='/hpc/pahm409/joint_training_no_transfer')
    ...
    BATCH_SIZE = 8
    PATCH_SIZE = (96, 96, 96)
    LEARNING_RATE = 0.001  # Same LR for both (no pretraining)
    WEIGHT_DECAY = 3e-5
    MOMENTUM = 0.99
    POLY_EXPONENT = 0.9
    ...
    print("="*80)
    print("JOINT TRAINING - FROM SCRATCH (NO TRANSFER LEARNING)")
    print("="*80)
    print(f"Architecture: Two separate models (DWI + ADC)")
    print(f"Initialization: Random (Kaimg)")
    print(f"Patch size: {PATCH_SIZE}")
    print(f"Batch size: {BATCH_SIZE}")
    print(f"Epochs: {EPOCHS}")
    print(f"Learning rate: {LEARNING_RATE} (same for both)")
    print("="*80 + "\n")
