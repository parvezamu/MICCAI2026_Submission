"""
train_segmentation_corrected_DS_MAIN_ONLY.py

Deep Supervision architecture but loss ONLY on main output
- Auxiliary heads exist (for gradient flow)
- But NO auxiliary losses (no gradient noise)
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, scaler, 
                deep_supervision=False, use_aux_loss=False,
                debug_first_batch=False, max_grad_norm=1.0):
    """
    Modified train_epoch:
    - If deep_supervision=True and use_aux_loss=False:
      DS architecture exists but loss computed ONLY on main output
    """
    model.train()
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        if debug_first_batch and batch_idx == 0:
            ...
            if deep_supervision:
                print(f"DS architecture: ENABLED")
                print(f"Use auxiliary losses: {use_aux_loss}")
            print("="*70 + "\n")
        ...
        with autocast():
            ...
            if deep_supervision:
                ...
                # Loss ONLY on main output (ignore auxiliary outputs!)
...
def main():
    parser = argparse.ArgumentParser(description='DS Architecture with Main-Only Loss')
    ...
    parser.add_argument('--deep-supervision', action='store_true',
                       help='Enable DS architecture (auxiliary heads exist for gradient flow)')
    ...
    parser.add_argument('--lr', type=float, default=0.0002,
                       help='Learning rate (default 0.0002 for batch 32)')
    ...
    print("\n" + "="*70)
    print(f"DS ARCHITECTURE + MAIN-ONLY LOSS - FOLD {args.fold}")
    print(f"Initialization: {init_method}")
    print(f"Attention: {attention_names[args.attention]}")
    if args.deep_supervision:
        print(f"Deep Supervision: ENABLED (architecture only, NO auxiliary losses)")
    else:
    ...
    ds_suffix = '_ds_main_only' if args.deep_supervision else ''
    ...
    print("\n" + "="*70)
    print(f"Starting Training - {init_method}")
    if args.deep_supervision:
        print("DS Architecture: YES (auxiliary heads exist)")
        print("Auxiliary Losses: NO (loss only on main output)")
    print("="*70 + "\n")
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_dsc = train_epoch(
            ...
            use_aux_loss=False,  # NO auxiliary losses!
    ...
    with open(exp_dir / 'final_summary.json', 'w') as f:
        json.dump({
            ...
            'attention_type': args.attention,
            'deep_supervision': args.deep_supervision,
            'use_auxiliary_loss': False,  # Always False in this version
