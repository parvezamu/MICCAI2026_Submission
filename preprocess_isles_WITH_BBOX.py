...
Same preprocessing as before BUT saves bbox coordinates
Use this ONLY for evaluation - don't touch training data!
"""
...
def process_isles_case(case_dir, output_dir, target_shape=(96, 96, 96)):
    ...
    try:
        ...
        if bbox is not None:
            ...
            cropped_shape = dwi_cropped.shape
        else:
        ...
        np.savez_compressed(
            ...
            bbox=np.array(bbox, dtype=np.int32) if bbox is not None else np.array([[-1,-1],[-1,-1],[-1,-1]], dtype=np.int32),
            original_shape=np.array(original_shape, dtype=np.int32),
            cropped_shape=np.array(cropped_shape, dtype=np.int32),
...
def main():
    ...
    print("="*80)
    print("PREPROCESSING WITH BBOX METADATA (FOR EVALUATION ONLY)")
    print("="*80)
    ...
    print(f"\n✓ Processed {success_count}/{len(case_dirs)} cases")
